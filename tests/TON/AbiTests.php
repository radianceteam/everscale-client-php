...
use TON\Abi\External;
use TON\Abi\FunctionHeader;
use TON\Abi\Keys;
use TON\Abi\MessageBodyType;
use TON\Abi\None;
...
class AbiTests extends AbstractModuleTestCase
{
    ...
    public function testEncodeMessage_deploy_external()
    {
        $unsigned = $this->_module->encodeMessage(
            self::getDeployParams(
                (new External())->setPublicKey($this->_keys->getPublic()),
    ...
    public function testAttachSignature_deployed()
    {
        ...
        $signed = $this->_module->attachSignature((new ParamsOfAttachSignature())
            ->setAbi((new Contract())->setValue($this->_abi))
    ...
    {
        $signing = (new Keys())->setKeys($this->_keys);
    ...
    {
        $unsigned = $this->_module->encodeMessage(
            self::getRunParams((new External())
                ->setPublicKey($this->_keys->getPublic())));
