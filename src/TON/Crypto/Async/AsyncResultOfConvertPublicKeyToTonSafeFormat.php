<?php

/**
 * This file is auto-generated.
 */

declare(strict_types=1);

namespace TON\Crypto\Async;

use TON\Crypto\ResultOfConvertPublicKeyToTonSafeFormat;
use TON\TonClientException;
use TON\TonRequest;

class AsyncResultOfConvertPublicKeyToTonSafeFormat
{
    /** TON request handle. */
    private TonRequest $_request;

    /**
     * AsyncResultOfConvertPublicKeyToTonSafeFormat constructor.
     * @param TonRequest $request Request handle.
     */
    public function __construct(TonRequest $request)
    {
        $this->_request = $request;
    }

    /**
     * Blocks until function execution is finished and returns execution result.
     * @return ResultOfConvertPublicKeyToTonSafeFormat Function execution result.
     * @throws TonClientException Function execution error.
     */
    public function await(): ResultOfConvertPublicKeyToTonSafeFormat
    {
        return new ResultOfConvertPublicKeyToTonSafeFormat($this->_request->await());
    }
}
